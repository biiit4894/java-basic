...
- 반복문 내에서만 사용, 반복 진행 준 만나면 반복문의 끝으로 가서 다음 반복으로 그냥 넘어감.
  - for문: 증감식으로 / while, do-while문: 조건식으로

# 23 이름 붙은 반복문

- 여러 개의 반복문이 중첩된 경우 → break문으로 중첩 반복문 탈출 불가
- 중첩 반복문 앞에 이름을 붙이고 break문, continue문에도 이름을 지정하기
